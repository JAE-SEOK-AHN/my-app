...
<script type="text/javascript">
	...
	function openDetailPopup(type) {
		...
		var template = [
			{bind:'userId', name:'userId',id:'userId', type:'text', label:'아이디', required:true, labelWidth:'80px', width:'250px'}
			, {bind:'name', name: 'name',id:'name', type:'text', label:'이름', required:true, labelWidth:'80px', width:'250px'}
...
/* 		$('#popupForm').jqxValidator({
			rules: [
				{ input: '#test', messsage: '아이디를 입력하세요.', action: 'keyup, blur', rule: 'required',position: 'right:0,0' }
				/* , { input: '#userId', messsage: '아이디를 입력하세요.', action: 'keyup, blur', rule: 'required',position: 'right:0,0' }
			 */
				]
		});	 */	
...
/* 		param.userId = $('#popupForm').jqxForm('getComponentByName','userId').val(); 
		...
		// 윤정 - 유효성 검사
		//$('#popupForm').jqxForm('getComponentByName','userId')[0].id = 'userId';
	...
	<div hidden="hidden">
		<div id="jqxwindow">
			...
			<div>
				<div id="popupForm">
				<!-- 유효성 검사 -->
					<!-- <input id="test" clss="classTest" type="text"/> -->
				</div>
